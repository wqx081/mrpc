...
//
// scoped_ptr<int, FreeDeleter> foo_ptr(
...
template <typename T> struct IsNotRefCounted {
  enum {
    value = !is_convertible<T*, RefCountedBase*>::value &&
        !is_convertible<T*, RefCountedThreadSafeBase*>::
            value
  };
};
...
class scoped_ptr_impl {
 public:
  ...
  scoped_ptr_impl(scoped_ptr_impl<U, V>* other)
    ...
    // emulation to have move() and forward()
...
class scoped_ptr {
  ...
  static_assert(mrpc::internal::IsNotRefCounted<T>::value,
 ...
 private:
  typedef mrpc::internal::scoped_ptr_impl<element_type, deleter_type>
      scoped_ptr::*Testable;
 ...
 private:
  ...
  mrpc::internal::scoped_ptr_impl<element_type, deleter_type> impl_;
